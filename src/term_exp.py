...
group by A.cycle_day, A.term_code, A.pidm, A.levl_code, A.styp_code"""
    ...
    def get_adm(self):
        ...
        qry = f"select A.* from {subqry(qry)} A where r = 1 and A.levl_code = 'UG' and A.styp_code in ('N','R','T')"
        qry = self.get_attributes(qry)

...
def train_ray(config, X_train, y_train, X_val, y_val):
    automl = AutoML()
    settings = {
        "task": "classification",
        "log_file_name": "",
    ...
    log_loss_val = log_loss(y_val, val_proba)

    return {"log_loss": log_loss_val, "model": automl}
...
class AMP(MyBaseClass):
    ...
    aggregations: tuple = ('crse_code', 'coll_desc', 'international')
    ...
    def get_y_pred(self):
        def func():
            ...
            pg = PlacementGroupFactory([{"CPU": 64, "GPU": 2}])
            ...
            for train_code in self.term_codes:
                ...
                if len(X_model) > 0:
                    ...
                    analysis = tune.run(
                        tune.with_parameters(train_ray, X_train=X_model[mask], y_train=y_model[mask], X_val=X_model[~mask], y_val=y_model[~mask]),
                        
                        config=search_space,
                        search_alg = BlendSearch(),
                        num_samples=1,
                        resources_per_trial=pg,
                        scheduler=ASHAScheduler(metric="log_loss", mode="min"),
                        # param_space = search_space
                    )
                    
                    
                    best_config = analysis.get_best_config(metric="log_loss", mode="min")
                    best_model_dict = train_ray(best_config, X_model[mask], y_model[mask], X_model[~mask], y_model[~mask])
                    best_model = best_model_dict['model']
                    pred = best_model.predict(Z.drop(columns=['actual', 'mask']))
                    proba = best_model.predict_proba(Z.drop(columns=['actual', 'mask']))[:, 1]

                    train_score[train_code] = best_model_dict['log_loss'] * 100
                    self.clf[train_code] = best_model._trained_estimator
    ...
    def get_y_stack(self):
        def func():
            stack = lambda key: pd.concat([read(fn).get(key, None) for fn in sorted((self.root_path / 'y_pred').rglob('*.pkl'))])
